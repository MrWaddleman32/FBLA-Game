...
var inventory = []
var items = {"Wood": TREE_LOG, "Planks": WOOD_PREVIEW_2}
...
func add(item):
	...
	#var checked_list = []
	#for i in range(len(inventory)):
	#	if inventory[i] in checked_list:
	#		inventory.remove_at(i)
	#		i -= 1
	#	else:
	#		checked_list.append(inventory[i])
	#		count.append(inventory.count(inventory[i]))
	print(inventory)
	print(item_and_count)
	for i in range(len(item_and_count.keys())):
